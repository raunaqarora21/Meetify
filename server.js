...
mongoose
  .connect(
    ...
    // || 'mongodb://localhost/meetify'
    "mongodb+srv://raunaq21:k91nszOUBFnre4rq@cluster0.fjxyt.mongodb.net/myFirstDatabase?retryWrites=true&w=majority",
    {
...
io.on("connection", (socket) => {
  socket.on("join-room", async (roomId, peerId, userId, name, audio, video) => {
    ...
    try {
      ...
      socket
        .to(roomId)
        .emit("user-connected", peerId, name, audio, video, roomData.count + 1);
      ...
      socket.on("disconnect", async () => {
        ...
        socket.to(roomId).emit("user-disconnected", peerId, roomData.count - 1);
      });
